...
export function GridListContent({
   ...
   * 1. if the id prop is provided, use it
   * 2. if the id is in context, use it
   * 3. otherwise, use useId();
   */

  const autoGeneratedId = useId();
  const contextualId = useGridListState().gridId;
  const actualId = id ?? contextualId ?? autoGeneratedId;
  ...
  return (
    <GridContentContext.Provider
      value={{ gridId: actualId, startRef, endRef, containerRef }}
    >
      {listInner}
    </GridContentContext.Provider>
  );
